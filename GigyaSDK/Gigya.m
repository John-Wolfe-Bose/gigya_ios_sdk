...
- (Gigya *)init
{
    ...
    if (self) {
        ...
        // attempt to load session from keychain
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            //set thread sleep delay to check keychain latency
...
{
    if (![session isEqual:self.session] || ![session.info isEqual:self.session.info]) {
...
- (void)getSessionWithCompletionHandler:(GSGetSessionCompletionHandler _Nonnull)handler
{
    if(!self.sessionLoaded) {
        // wait no longer, than 5 sec for keychain, then consider fail
        dispatch_time_t dT = dispatch_time(DISPATCH_TIME_NOW, 5 * NSEC_PER_SEC);
        long waitResult = dispatch_semaphore_wait(self.semaphore, dT);
        if(waitResult != 0) {
            self.sessionLoaded = YES;
...
- (BOOL)isSessionValid
{
    if (!self.sessionLoaded) {
        ...
        dispatch_time_t dT = dispatch_time(DISPATCH_TIME_NOW, 5 * NSEC_PER_SEC);
        long waitResult = dispatch_semaphore_wait(self.semaphore, dT);
        if (waitResult != 0 || ![self.session isValid]) {
            self.sessionLoaded = YES;
            [self deleteSessionFromKeychain];
            return NO;
        } else {
...
- (void)deleteSessionFromKeychain
{
    [KeychainStorage removeDataForAttributeName:GSSessionKeychainKey
                                    serviceName:GSGigyaSDKDomain
                              completionHandler:nil];
