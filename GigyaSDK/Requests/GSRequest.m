    ...
    GSGetSessionCompletionHandler sessionHandler = ^(GSSession *session) {
        ...
        if (forceNoAuth) {
            ...
            if (self.includeAuthInfo && self.session && [self.session isValid]) {
                ...
                (self.parameters)[@"sig"] = [GSignatureUtils oauth1Signature: requestURL session: session parameters: self.parameters];
            }
        }
        ...
        // Create the request
        NSLog(@"Request URL: %@?%@", requestURL, [self.parameters GSURLQueryString]);
    ...
    if (!error && (sessionInfo || response[@"sessionToken"])) {
        ...
        if (token && secret) {
            ...
            GSSession *newSession = [[GSSession alloc] initWithSessionToken:token
                                                                     secret:secret
                                                                  expiresIn:expiresIn];
            NSString *lastLoginProvider = response[@"loginProvider"];
            if ([lastLoginProvider isEqualToString:@"site"]) {
                newSession.lastLoginProvider = @"site";
            }
