...
- (void)pluginView:(GSPluginView *)pluginView didFailWithError:(NSError *)error
{
    ...
    if (error.code == GSErrorJSException || error.code == GSErrorLoadFailed) {
        GSLog(@"Network error: %d", error.code);
        ...
        // remove the delegate if have network issue
        self.pluginView.delegate = nil;
    }
}
