...
+ (BOOL)canUseSession {
    // check for app's reinstallation, and delete session from KeyChain if so
    NSFileManager *fileManager =  [NSFileManager defaultManager];
    NSString *foldDestination = BASEPATH;
    NSString *filePath = [foldDestination stringByAppendingPathComponent: @REINSTFNAME];
    BOOL fileExist = [fileManager fileExistsAtPath:filePath];
    
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    BOOL installArtifacts = [defaults objectForKey:GSUCIDDefaultsKey] != NULL || [defaults objectForKey:GSGMIDDefaultsKey] != NULL;
    
    // check for installation evidence
    if (!fileExist) {
        // create temp file if not present
        if (![fileManager fileExistsAtPath:foldDestination isDirectory:NULL]) {
            NSError *error = nil;
            if (![fileManager createDirectoryAtPath:foldDestination withIntermediateDirectories:YES attributes:nil error:&error]) {
            }
        }
              ...
              completionHandler:(KeychainStorageGetDataHandler _Nonnull)handler
{
    ...
    // check for app's reinstallation, and delete session from KeyChain if so
    if (![self canUseSession]){
        // if no evidence of old SDK in userDefaults - consider reistallation -> remove session from keyChain
        [self removeDataForAttributeName:attributeName
                             serviceName:serviceName
                       completionHandler:^(NSError * _Nullable error) {
                           
                           NSError *reinstErr = [NSError errorWithDomain:GSGigyaSDKDomain
                                                                    code:-1
                                                                userInfo:@{ NSLocalizedDescriptionKey: @"Reinstallation detected, session data erased" }];
                           handler(nil, reinstErr);
                       }];
        return;
    }
