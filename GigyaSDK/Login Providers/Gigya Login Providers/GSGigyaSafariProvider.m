...
@property (strong, nonatomic) SFSafariViewController *safariViewContoller;
    ...
    [Gigya getSessionWithCompletionHandler:^(GSSession *session) {
        NSURL *loginURL = [NSURL URLForGigyaLoginMethod:method
                                      ...
                                      redirectURLScheme:[self URLScheme]
                                                session:session];
        if (@available(iOS 9, *)) {
            self.safariViewContoller = [[SFSafariViewController alloc] initWithURL:loginURL];
            self.safariViewContoller.delegate = self;
            UIViewController *currentViewController = [[UIApplication sharedApplication].keyWindow rootViewController];
            [currentViewController presentViewController:self.safariViewContoller animated:YES completion:nil];
        } else {
...
{
    [self.safariViewContoller dismissViewControllerAnimated:YES completion:nil];
    if (self.handler) {
