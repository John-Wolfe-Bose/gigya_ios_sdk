...
{
    //User did not allow access to Twitter Accounts
    if(error.code == 403007 ) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self handleTwitterPermissionError];
        });
    }
    if (self.handler) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
#pragma mark - Show alert when no Twitter permissions
-(void)handleTwitterPermissionError {
    
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Provider permission denied"
                                                    message:@"We don't have permissions to complete the login.\nPlease close the app, enable permissions at:\nSettings -> Privacy -> Twitter and try again."
                                                   delegate:nil
